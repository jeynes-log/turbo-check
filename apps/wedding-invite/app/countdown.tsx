  ...
  return (
    <div className="flex flex-col items-center gap-y-2">
      <div className="flex justify-center gap-4">
        ...
        <div className="flex flex-col items-center">
          <span className="text-2xl font-bold text-stone-800 sm:text-3xl">{pad(diff.minutes)}</span>
          <span className="text-xs text-stone-500">분</span>
        </div>
        <div className="flex flex-col items-center">
          <span className="text-2xl font-bold text-stone-800 sm:text-3xl">{pad(diff.seconds)}</span>
      ...
      <p className="text-sm font-medium text-stone-600">
        {diff.days === 0 && diff.hours === 0 && diff.minutes === 0 && diff.seconds === 0 ? (
          ...
          <span>
            <span className="font-bold text-pink-500">{diff.days}</span>일 남았습니다.
          </span>
        )}
      </p>
    </div>
